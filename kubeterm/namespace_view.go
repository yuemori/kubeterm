...
func NewNamespaceView(client *Client) *NamespaceView {
	...
	table.AddHeader([]string{"Name", "Status", "CreationTimestamp"})
	...
	for _, ns := range nss.Items {
		table.AddRow([]string{ns.ObjectMeta.Name, string(ns.Status.Phase), ns.ObjectMeta.CreationTimestamp.Time.String()})
	}
...
func (v *NamespaceView) Height() int {
